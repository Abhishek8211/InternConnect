...
import { 
  Briefcase, MapPin, Users, Eye, Edit3, Trash2, 
  MoreHorizontal, Plus, Search, CheckCircle2, Clock, 
  XCircle, Calendar 
} from "lucide-react";
...
const ManageListings = () => {
  ...
  const fetchListings = useCallback(async () => {
    try {
      setLoading(true);
      const res = await internshipService.getMyListings();
  ...
  const toggleStatus = async (id, currentStatus) => {
    try {
      ...
      // Actually we just toggle isActive bool on backend usually, but let's see. 
      // The schema has isActive boolean. Let's send { isActive: ... }
      const isActive = newStatus === "active";
      await internshipService.update(id, { isActive });
      setListings((p) => p.map((l) => l._id === id ? { ...l, isActive } : l));
      setOpenMenu(null);
    } catch (err) {
  ...
  // Maps backend isActive & isExpired to our local UI states
  const getJobStatus = (job) => {
    if (!job.isActive) return "closed";
    if (new Date(job.applicationDeadline) < new Date()) return "closed";
    return "active"; // "draft" is not strictly in the backend, we just use active/closed
  };

  const filtered = listings.filter((l) => {
    const s = getJobStatus(l);
  ...
  const counts = { 
    all: listings.length, 
    active: listings.filter(l => getJobStatus(l) === "active").length, 
    closed: listings.filter(l => getJobStatus(l) === "closed").length 
  };
  ...
  return (
    <div className="min-h-full p-4 sm:p-6 lg:p-8 animate-fade-in">
      ...
      {/* Table */}
      <div className="rounded-2xl border border-white/[0.05] bg-[#111827] overflow-hidden min-h-[300px]">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-white/[0.03]">
              {filtered.length === 0 ? (
                ...
                const cfg = STATUS_CFG[jobStatus];
                const SI = cfg.icon;
                  ...
                  <tr key={job._id} className="group transition-colors hover:bg-white/[0.02]">
                    <td className="px-6 py-4">
                      <div>
                        ...
                        <p className="mt-0.5 flex items-center gap-1 text-xs text-slate-500"><MapPin className="h-3 w-3" />{job.location?.city}, {job.location?.country}</p>
                        <p className="text-[11px] text-slate-600 mt-0.5">Posted {formatDistanceToNow(new Date(job.createdAt), { addSuffix: true })}</p>
                      </div>
                    </td>
                    <td className="px-5 py-4">
                      <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${TYPE_COLORS[job.type] || "bg-slate-500/15 text-slate-400"}`}>{job.type}</span>
                    </td>
                    <td className="px-5 py-4 text-sm text-slate-300">
                      {job.stipend?.amount ? `₹${job.stipend.amount}/mo` : "Unpaid"}
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-semibold text-white">{job.applicationCount || 0}</span>
                      </div>
                      <div className="mt-1 h-1 w-20 rounded-full bg-white/10">
                        <div className="h-1 rounded-full bg-gradient-to-r from-violet-500 to-cyan-500"
                          style={{ width: `${Math.min(100, (job.applicationCount || 0) * 5)}%` }} />
                      </div>
                    </td>
                    <td className="px-5 py-4">
                      <span className="flex items-center gap-1 text-sm text-slate-400"><Eye className="h-3 w-3" />{job.viewCount || 0}</span>
                    </td>
                    <td className="px-5 py-4">
                      <span className="flex flex-col text-xs text-slate-400">
                        <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />{format(new Date(job.applicationDeadline), "MMM dd, yyyy")}</span>
                      </span>
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4 relative">
                      ...
                      {openMenu === job._id && (
                        <div className="absolute right-12 top-3 z-20 w-44 rounded-xl border border-white/10 bg-[#1a2540] shadow-xl py-1.5">
                          <Link to={`/recruiter/edit/${job._id}`} className="flex w-full items-center gap-2.5 px-4 py-2 text-xs text-slate-400 hover:bg-white/5 hover:text-white transition-all"><Edit3 className="h-3.5 w-3.5" />Edit</Link>
                          <button onClick={() => toggleStatus(job._id, jobStatus)}
                            ...
                            {jobStatus === "active" ? <><XCircle className="h-3.5 w-3.5" />Close listing</> : <><CheckCircle2 className="h-3.5 w-3.5" />Reactivate</>}
                          </button>
                          <button onClick={() => remove(job._id)} className="flex w-full items-center gap-2.5 px-4 py-2 text-xs text-red-400 hover:bg-red-500/10 transition-all"><Trash2 className="h-3.5 w-3.5" />Delete</button>
                        </div>
                      )}
                    </td>
                  </tr>
